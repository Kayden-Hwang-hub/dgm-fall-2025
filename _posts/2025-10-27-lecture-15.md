...
- Estimates:
  - Parameters are learned by maximizing the conditional likelihood: $\hat{\theta}_{LR} = \arg\max_\theta P(Y \mid X; \theta)$
  - Or equivalently, by maximizing the **log-likelihood**:
    $$\hat{\theta}_{LR} = \arg\max_\theta \sum_i \Big[Y_i \log \sigma(\theta^T X_i) + (1 - Y_i)\log(1 - \sigma(\theta^T X_i))\Big]$$
- Properties:
